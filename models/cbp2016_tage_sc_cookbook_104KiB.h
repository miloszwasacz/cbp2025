...
// file main.cc was  modified to stop the simulation after 10000000 branches for all benchmarks and to potentially accomodate the option WARM
...
//#define RANDINIT  // RANDINIT provide random values in all counters, might be slightly more realistic than initialization with weak counters
...
#define MAXBR 4 // Maximum MAXBR  branches in  the block; the code assumes MAXBR is a power of 2
#define NBREADPERTABLE 4 	//predictions read per table for a block

// general prameters
#define AHEAD 0// in the curent version:  only 0 or 2 are valid (0 corresponds to the conventional 1-block ahead, 2 coresponds to the 3-block ahead)
...
//#define INTERLEAVED // just to show that it  is not  fully interleaving the banks is not that great and probably not worth the  extra 14x14 shuffling/reshuffling
#ifdef INTERLEAVED
#define SHARED 0
...
#define PSK 1
#define REPSK 1 // this optimization is funny, if no "useless" entry, move the entry on the other way to make room, brings a little bit of accuracy
#else
#define PSK 1
#define REPSK 0
#endif
...
#define UPDATEALTONWEAKMISP 1	// When the Longest match is weak and wrong, one updates also the alternate prediction and HCPred : ~0.018 MPKI
...
#define FORCEONHIGHCONF  //   if TAGE is high conf and SC very low conf then use TAGE, if SC: brings 0.008 - 0.016 MPKI, but a 5-to-1 mux instead a 4-to-1
//#define MORESCLOGICAHEAD // if AHEAD and if SC uses four times  the number of adder trees (compute 16 SCsum  per prediction !),  ~ 1 % gain in accuracy
...
#define INDBIASPC (((((PCBL ^(PCBL>>(LOGBIAS-5))))) & ((1<<LOGBIAS) -1)) ^ PSNUM)
#define INDBIASPCLMAP (INDBIASPC) ^((LongestMatchPred  ^(HCpred <<1))<< (LOGBIAS-2))
// a single  physical table but  two logic tables: indices agree on all the bits except 2


#define INDBIASBHIST (((((PCBL ^PrevBHIST ^(PCBL>>(LOGBIAS-4))))) & ((1<<LOGBNB) -1)) ^ PSNUM)
#define INDBIASFHIST (((((PCBL  ^PrevFHIST ^(PCBL>>(LOGBIAS-3))))) & ((1<<LOGFNB) -1)) ^ PSNUM)
#define INDBIASIMLIBR (((((PCBL ^PrevF_BrIMLI ^(PCBL>>(LOGBIAS-6))))) & ((1<<LOGINB) -1)) ^ PSNUM)
#define INDBIASIMLITA ((((((PCBL >> 4) ^PrevF_TaIMLI ^(PCBL<< (LOGBIAS-4))))) & ((1<<LOGINB) -1)) ^ PSNUM)
     ...
     void reinit ()
          {
               ...
               for (int i = 3; i <= NHIST; i++)
                    if (m[i] <= m[i - 1])
                         m[i] = m[i - 1] + 1;
#endif
               if ((AHEAD!=0) & (AHEAD!=2)) exit(1); // prediction is considered to be done in 1 cycle or 3 cycles
               ...
               { btable[i].pred= random () &1; btable[i].hyst= random() & 3;
               }
               updatethreshold = 23;
#ifdef SCMEDIUM
               for (int j = 0; j < ((1 << LOGBNB) - 1); j++)
...
// gindex computes a full hash of PC, ghist and phist
     uint gindex (unsigned int PC, int bank, long long hist,
                  folded_history * ch_i)
          {
               uint index;
     ...
     void Tagepred (uint64_t PC)
          {
               ...
               BI = (PCBLOCK ^((Numero & (NBREADPERTABLE - 1))<< (LOGB-2))) & ((1 << LOGB) - 1);
...
//For ahead, one considers that the bimodal prediction is  obtained during the last cycle
               for (int i = 1; i <= NHIST; i++)
               {
...
#else
                    {
                         GI[i] =
                              AHGI[(NPRED -
                                    AHEAD) % 10][i] ^ ((Numero & (NBREADPERTABLE - 1)) <<  (LOGG-LOGASSOC-2));
                         GI[i] *= ASSOC;
...
#ifndef INTERLEAVED
#ifdef ADJACENTTABLE
               for (int i = 2; i <= NHIST; i += 2)                     GI[i] = GI[i - 1] ;
               ...
               for (int i = 1; i <= NHIST; i++)
               {
                    ...
                    if (PSK==1){
                         ...
                         if (AHEAD==0)
                              for (int j=1;j<ASSOC;j++)    GGI[j][i]^= ((GTAG[i] >> (3+2*j)) & 0x3) << (LOGG-3);
                         else

                              for (int j=1;j<ASSOC;j++)  GGI[j][i]^= ((GTAG[i] >> (3+2*j)) & (READWIDTHAHEAD-1)) << (LOGG-5);
                    }
...
// computes the TAGE table addresses and the partial tags
               ...
               return pred_taken;
#endif
               if (AHEAD)   PC = PrevPCBLOCK ^ (Numero << 5) ^ (PrevNumero<<5) ^((BI & 3) <<5);
...
// x 2: a little bit better
     ...
     void HistoryUpdate (uint64_t PCBRANCH, InstClass opType, bool taken,
                         uint64_t branchTarget,  int &Y,
...
// rely on close target  and on close  branches : capture loop nests, see slide set
                    ...
                    int PATH =
                         PC ^ (PC >> 2) ^ (PC >> 4) ^ (branchTarget) ^ (Numero << 3);
                    phist = (phist << 4) ^ PATH;
                    phist = (phist & ((1 << 27) - 1));
...
// PREDICTOR UPDATE
     ...
     void UpdatePredictor (uint64_t PCBRANCH, InstClass opType,
                           bool resolveDir, bool predDir, uint64_t branchTarget)
          {
...
//TAGE UPDATE
               ...
               if (HitBank>0)             if ((TAGECONF==0) ||( (MYRANDOM() & 3)==0))               ctrupdate(CountLowConf, (TAGECONF==0), 7);
                    ...
                    if (PseudoNewAlloc)
                    {
                         ...
                         for (int i= HitBank/4; i<=NHIST/4;i++)
                         {
                              ...
                              if ((LongestMatchPred != resolveDir) || ((MYRANDOM () & 31) > 1))
                                   ctrupdate (COUNT16_31[i], (resolveDir == LongestMatchPred), 7);	// more or less than 16/31  good predictions on weak counters
                         }
...
//END TAGE UPDATE
               HistoryUpdate (PCBRANCH, opType, resolveDir,
                              branchTarget,  ptghist, ch_i, ch_t[0], ch_t[1]);
//END PREDICTOR UPDATE
          }



     void TrackOtherInst (uint64_t PCBRANCH, InstClass opType, bool taken,
                          uint64_t branchTarget)
          {
               HistoryUpdate (PCBRANCH, opType, taken, branchTarget,
